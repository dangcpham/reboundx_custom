...
from .extras import Param, Node, Force, Operator, Extras, REBX_CTYPES, Flybys
...
class Params(MutableMapping):
    ...
    def __setitem__(self, key, value):
        ...
        if ctype == c_void_p:
            clibreboundx.rebx_set_param_pointer(self.rebx, byref(self.ap), c_char_p(key.encode('ascii')), byref(value))
        if ctype == Flybys:
            if not isinstance(value, Flybys):
                raise AttributeError("REBOUNDx Error: Parameter '{0}' must be assigned a Flybys object.".format(key))
            clibreboundx.rebx_set_param_pointer(self.rebx, byref(self.ap), c_char_p(key.encode('ascii')), byref(value))
