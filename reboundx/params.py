...
from .extras import Param, Node, Force, Operator, Extras, REBX_CTYPES, Planets
...
class Params(MutableMapping):
    ...
    def __setitem__(self, key, value):
        ...
        if ctype == rebound.ODE:
            ...
            clibreboundx.rebx_set_param_pointer(self.rebx, byref(self.ap), c_char_p(key.encode('ascii')), byref(value))
        if ctype == Planets:
            if not isinstance(value, Planets):
                raise AttributeError("REBOUNDx Error: Parameter '{0}' must be assigned a Planets object.".format(key))
            clibreboundx.rebx_set_param_pointer(self.rebx, byref(self.ap), c_char_p(key.encode('ascii')), byref(value))
        if ctype == c_void_p:
            clibreboundx.rebx_set_param_pointer(self.rebx, byref(self.ap), c_char_p(key.encode('ascii')), byref(value))
