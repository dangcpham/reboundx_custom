...
class Planets(Structure):
    def __new__(cls, rebx, Nplanets, planets_data, masses):
        planets = super(Planets, cls).__new__(cls)
        return planets
    ...
    def __del__(self):
        if self._b_needsfree_ == 1:
            clibreboundx.rebx_free_planets_pointers(byref(self))

Planets._fields_ = [ ("planets_data", POINTER(c_double)), 
...
REBX_C_TO_CTYPES = [["REBX_TYPE_NONE", None], ["REBX_TYPE_DOUBLE", c_double], ["REBX_TYPE_INT",c_int], ["REBX_TYPE_POINTER", c_void_p], ["REBX_TYPE_FORCE", Force], ["REBX_TYPE_UNIT32", c_uint32], ["REBX_TYPE_ORBIT", rebound.Orbit], ["REBX_TYPE_ODE", rebound.ODE], ["REBX_TYPE_VEC3D", rebound._Vec3d], ["REBX_TYPE_PLANETS", Planets]]
